...
public class UpdateAcc extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int accId=Integer.parseInt(request.getParameter("id").toString());
        if (type==0){
            dao.deleteAcc(accId);
        }else {
            request.setAttribute("accId", accId);
            for (Account account : dao.getAccount()) {
                if (account.getId()==accId){
                    request.setAttribute("accUpd", account);
                    dao.updateStatus(accId, (account.isStatus())?0:1);
                    break;
                }
            }
